...
class FranchiseController extends Controller{
    ...
    public function index(): void {
        ...
        $sql = "SELECT DISTINCT
                ...
                FROM franchises f
                LEFT JOIN (SELECT * FROM franchise_history WHERE NewStatus IN('new','renew')) fh ON f.id = fh.FranchiseID
        ...
        if ($year) {
            $sql .= " AND YEAR(fh.DateIssued) = ?";
            $params[] = $year;
        }
        
        if ($search) {
            $sql .= " AND (f.FranchiseNo LIKE ? OR fh.PlateNo LIKE ? OR fh.ORNo LIKE ? OR a.Address LIKE ? OR 
                    a.ContactNo LIKE ? OR fh.DateIssued LIKE ? OR f.ChassisNo LIKE ? OR f.EngineNo LIKE ? OR 
                    ff.DropReason LIKE ? OR ff.ExpiryDate LIKE ? or f.LastRenewalDate LIKE ? OR a.FirstName LIKE ? OR a.LastName LIKE ?)";
            $searchParam = "%{$search}%";
            $params[] = $searchParam;
            $params[] = $searchParam;
            $params[] = $searchParam;
        ...
        if ($year) {
            $countSql .= " AND YEAR(fh.DateIssued) = ?";
            $countParams[] = $year;
        }
        if ($search) {
            $countSql .= " AND (f.FranchiseNo LIKE ? OR ff.PlateNo LIKE ? OR fh.ORNo LIKE ? OR a.Address LIKE ? OR 
                    a.ContactNo LIKE ? OR fh.DateIssued LIKE ? OR f.ChassisNo LIKE ? OR f.EngineNo LIKE ? OR 
                    fh.DropReason LIKE ? OR fh.ExpiryDate LIKE ? or f.LastRenewalDate LIKE ? OR a.FirstName LIKE ? OR a.LastName LIKE ?)";
            $searchParam = "%{$search}%";
        ...
        $sql .= " ORDER BY fh.DateIssued DESC, f.CreatedAt DESC LIMIT ? OFFSET ?";
        $params[] = $limit;
        $params[] = $offset;
	...
	$lastHistory = Database::fetch("SELECT * FROM franchise_history WHERE FranchiseID = ? AND NewStatus IN('new','renew') ORDER BY CreatedAt DESC LIMIT 1", [$id]);
