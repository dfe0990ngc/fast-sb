...
function Sidebar({
  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "flex flex-col bg-sidebar h-full text-sidebar-foreground",
          className,
        )}
        style={{
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="[&>button]:hidden bg-sidebar p-0 text-sidebar-foreground"
          style={
            {
  ...
  return (
    <div
      className={`group peer ${state === "collapsed" ? '!hidden':''} md:block text-sidebar-foreground`}
      data-state={state}
    ...
    >
      ...
      <div
        ...
        className={cn(
          "hidden z-10 fixed inset-y-0 md:flex h-svh transition-[left,right,width] duration-200 ease-linear",
          side === "left"
